...
const Products = ({ name, image, unit_amount, id, description, quantity }: ProductType) => {
  ...
  const secondImages = [image, imageS1, imageS2]
      ...
      <div className="w-[89%] m-auto max-w-[1400px] grid grid-cols-1 md:grid-cols-2 items-center gap-5">
        ...
        <div className="flex gap-4 items-center">
          <div className="flex flex-col gap-4">
            {(name === "Saturn Moonlamp" ? secondImages : productImages).map((img, index) => (
              <Image
                key={index}
                src={img}
                width={100}
          ...
          <div className="flex items-center">
            <motion.div initial="exit" animate="enter" exit="exit" variants={imageVariants} key={currentImage}>
              <Image
                src={name === "Saturn Moonlamp" ? secondImages[currentImage] : productImages[currentImage]}
                width={480}
                height={480}
                alt="main-image"
        ...
        <div className="text-center">
          ...
          <div className="mb-5">
            <span className="text-2xl mr-3 font-bold">${unit_amount && unit_amount / 100}</span>

            <span className="text-gray-400 line-through">${unit_amount && (unit_amount + 2000) / 100}</span>
          </div>
          <p className="mb-5">
            {description !== "n/a" ? description :
              "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Impedit quos, dolor eligendi et amet culpa veritatis! Perspiciatis fugiat fuga in ipsam. Natus voluptatibus quae laudantium"}
          </p>
